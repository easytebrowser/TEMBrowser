...
+function()
{
	...
	function SPEED_Y(){return 5 + RND() * 2}
	...
	CreateBubble = function(ppcId, bannerUrl)
	{
		var bub = new Bubble(ppcId, bannerUrl);
	 ...
	 * Class Bubble
	 ******************************/
	function Bubble(ppcId, bannerUrl)
	{
		var kOpa = [], kStp = [];
		var arrFlt = [];
		...
		for(var i=0; i<4; i++)
		{
			...
			{
				if(_IE6_)
					sty.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src=" + bannerUrl + ")";
				else
					sty.backgroundImage = "url(" + bannerUrl + ")";
				break;
			}
